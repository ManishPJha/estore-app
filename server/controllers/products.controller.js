...
const createProduct = catchAsyncErrors(async (req, res, next) => {
  ...
  let url = file.path.replace("uploads", "");

  const isFileExist = await Product.aggregate([
    {
      $match: {
        cgvURL: url,
      },
  ...
  if (isFileExist && isFileExist.length > 0)
    return next(new ErrorHandler("File is already exist. Try again!", 404));

  const product = await Product.create({ ...req.body, cgvURL: url });
...
const createProductReview = catchAsyncErrors(async (req, res, next) => {
  ...
  product.ratings =
    product.productReviews.reduce((acc, item) => item.rating + acc, 0) /
...
const removeProductReview = catchAsyncErrors(async (req, res, next) => {
  ...
  const reviews = product.productReviews.filter(
